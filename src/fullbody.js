...
class FullBodyExercise {
  ...
  startExercise() {
    ...
    selector('#output-container').innerHTML = preparation;

    //call preparation from index js
    prep(exercises[0].fullbodyworkout[index].image);

    //call start state after 20s
    setTimeout(() => {
      selector('#output-container').innerHTML = start;
      ...
      selector('#start-img').setAttribute(
        'src',
        exercises[0].fullbodyworkout[index].image
      );

      //set title
      selector('#title').textContent =
        exercises[0].fullbodyworkout[index].title;

      //set time or number of times for exercise
      selector('#times').textContent =
        exercises[0].fullbodyworkout[index].times;

      //set counter if exercise include time
      if (exercises[0].fullbodyworkout[index].times.includes('s')) {
        //remove s from time
        exercises[0].fullbodyworkout[index].times.slice(
          exercises[0].fullbodyworkout[index].times.length - 1
        );
        //call counter function from index.js
        counter(
          selector('#times'),
          parseInt(exercises[0].fullbodyworkout[index].times)
        );
      } else {
        selector('#times').textContent =
          exercises[0].fullbodyworkout[index].times;
  ...
  nextExercise() {
    ...
    if (index === exercises[0].fullbodyworkout.length) {
      selector('#output-container').innerHTML = greeting;
      setTimeout(() => {
        location.reload();
      }, 3000);
    }
    ...
    selector('#title').textContent = exercises[0].fullbodyworkout[index].title;

    selector('#times').textContent = exercises[0].fullbodyworkout[index].times;
  ...
  showPreview(id) {
    ...
    selector('#img-preview').setAttribute(
      'src',
      exercises[0].fullbodyworkout[previewIndex].image
    );

    //set preview title
    selector('#preview-title').textContent =
      exercises[0].fullbodyworkout[previewIndex].title;

    //set preview info
    selector('#preview-info').textContent =
      exercises[0].fullbodyworkout[previewIndex].info;
  ...
  nextPreview() {
    ...
    if (previewIndex >= exercises[0].fullbodyworkout.length) {
      previewIndex = 0;
    }
  ...
  previousPreview() {
    ...
    if (previewIndex < 0) {
      previewIndex = exercises[0].fullbodyworkout.length - 1;
    }
